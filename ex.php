...
class mypdf extends FPDF{
    function Header(){
        ...
        $text = "Date: 20/05/2022";
        $this->SetXY(197-($this->GetStringWidth($text)),90);
        $this->Cell(0,6,$text,0,'L');

        $this->setTextColor(255,51,51);
        $this->SetFont('DS','',25);
        $text = "Roonal Khandelwal";
        $this->Text(105 - ($this->GetStringWidth($text) / 2), 135, $text);
        $this->setTextColor(0,0,0);

        $this->SetXY(10,145);
        $this->SetFont('Arial','B',11);
        $text = "for the experience gained in our organization. As the head of out HR department, I hereby testify that this employee has worked in our company from 20/05/2022 to 20/05/2022 and has gained experience as a Developer of the Software Department";
        $this->MultiCell(190, 6, $text,0,'C');
